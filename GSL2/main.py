...
def get_parameters():
    ...
    parser.add_argument('--n_vertex', type=int, default=53)
    parser.add_argument('--max_len', type=int, default=80)
    parser.add_argument("--graph_skip_conn", type=float, default=0.09997491581800291, help="Default is 10.")
    ...
    parser.add_argument("--graph_metric_type", type=str, default="weighted_cosine", help="gat_attention, attention, weighted_cosine, kernel, transformer, cosine, mlp, multi_mlp")
    ...
    parser.add_argument("--top_k", type=int, default=4, help="Default is 10.")
    parser.add_argument("--epsilon", type=float, default=0.45924889196586727, help="Default is 10.")
    parser.add_argument('--steam_dim', type=int, default=3)
    parser.add_argument('--dhs_dim', type=int, default=3)
    ...
    parser.add_argument('--battery_dim', type=int, default=2)
    ...
    parser.add_argument('--droprate', type=float, default=0.41187640010910787)
    parser.add_argument('--lr', type=float, default=0.0003918194347141745, help='learning rate')
    parser.add_argument('--weight_type', type=int, default=3, help='1 for none, 2 for *, 3 for /')
    parser.add_argument('--weight_decay_rate', type=float, default=0.00023273922280628724,
...
def data_preparate_battery(GATG, GATP):
    G_df, P_df = get_data.get_node_data_ele(GATG, GATP)
    data_col = G_df.shape[0]
    val_and_test_rate = args.valid_ratio
    len_val = int(math.floor(data_col * val_and_test_rate))
    ...
    if args.use_eos == True:
        with open('data/eos/data_battery.pickle', 'wb') as f:
            pickle.dump(data, f)
        with open('data/eos/score_battery.pkl', 'wb') as f:
            pickle.dump(S_score, f)
    else:
        with open('data/noeos/data_battery.pickle', 'wb') as f:
            pickle.dump(data, f)
        with open('data/noeos/score_battery.pkl', 'wb') as f:
            pickle.dump(S_score, f)


def data_preparate_DHS(GATG, GATP, GATT):
    G_df, P_df, T_df = get_data.get_node_data_steam(GATG, GATP, GATT)
    ...
    if args.use_eos == True:
        with open('data/eos/data_DHS.pickle', 'wb') as f:
            pickle.dump(data, f)
        with open('data/eos/score_DHS.pkl', 'wb') as f:
            pickle.dump(S_score, f)
    else:
        with open('data/noeos/data_DHS.pickle', 'wb') as f:
            pickle.dump(data, f)
        with open('data/noeos/score_DHS.pkl', 'wb') as f:
            pickle.dump(S_score, f)
...
def train(score_list, loss, args, optimizer, scheduler, es, model, train_iter_list, val_iter_list):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        model.train()
        for batch_steam, batch_air, batch_ele, batch_c1, batch_c2, batch_DHS, batch_battery in tqdm(
                zip(*train_iter_list)):
            xs = batch_steam.x
            xa = batch_air.x
            xe = batch_ele.x
            ...
            if args.use_eos:
                ...
                xd_pred = utility.apply_stability_mask_eos(xd_pred, point_d)
                xb_pred = utility.apply_stability_mask_eos(xb_pred, point_b)
...
def val(model, val_iter_list, loss, score_list):
    ...
    for batch_steam, batch_air, batch_ele, batch_c1, batch_c2, batch_DHS, batch_battery in tqdm(zip(*val_iter_list)):
        xs = batch_steam.x
        xa = batch_air.x
        xe = batch_ele.x
        xc1 = batch_c1.x
...
def objective(params):
    ...
    data = ['data_steam.pickle', 'data_air.pickle', 'data_ele.pickle', 'data_c1.pickle', 'data_c2.pickle',
            'data_DHS.pickle', 'data_battery.pickle']
    score = ['score_steam.pkl', 'score_air.pkl', 'score_ele.pkl', 'score_c1.pkl', 'score_c2.pkl', 'score_DHS.pkl',
             'score_battery.pkl']
...
if __name__ == "__main__":
    ...
    data_preparate_DHS("./data/dataset/DHS/Flow", "./data/dataset/DHS/Pressure", "./data/dataset/DHS/Temperature")
    ...
    data_preparate_battery("./data/dataset/Battery/Capacity", "./data/dataset/Battery/Current")
    ...
    score_list, train_iter_list, val_iter_list, test_iter_list = load_data(args, data, score)
    loss, es, model, optimizer, scheduler = prepare_model(args, blocks)
    model.load_state_dict(torch.load(f'checkpoint/model_epoch_wgtcos_att_10.pth'))
    #train(score_list, loss, args, optimizer, scheduler, es, model, train_iter_list, val_iter_list)
    test(score_list, loss, model, test_iter_list, args, logger)

    # result = gp_minimize(objective, search_space, n_calls=40, random_state=args.seed, callback=my_callback)
