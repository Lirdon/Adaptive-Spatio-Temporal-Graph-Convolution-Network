...
class STGCNGraphConv(nn.Module):
    def __init__(self, args, blocks):
        ...
        steam1, steam2, air1, air2, ele1, ele2, ele3, edge_index = selected_pipe()
        ...
        self.conc = nn.Parameter(
            torch.nn.functional.normalize(
                torch.randn(7, dtype=torch.float32, requires_grad=args.need_grad).to(self.device),
                p=2, dim=0
            )
        )
        ...
        self.output_layer = my_layers.OutputLayer(args.graph_dim, [args.steam_dim, args.c1_dim, args.ele_dim, args.c2_dim, args.air_dim])

    def forward(self, xs, xa, xe, xc1, xc2, use_teacher_forcing):
        ...
        edge_weight = torch.cat([steam_weight, ele_weight, air_weight, self.conc], dim=0)#这里还要补充中间边和正确维度
        ...
        x = torch.cat((xs, xc1, xe, xc2, xa), dim=3)
