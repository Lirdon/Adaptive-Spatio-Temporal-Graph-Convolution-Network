...
class GraphConvLayer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        return outputs, edge_index, edge_weight
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, args, last_block_channel, channels):
        ...
        if args.time_model == 'attention':
            ...
            #def __init__(self, input_dim, hidden_dim, output_dim, seqlen, node):
    ...
    def forward(self, x, edge_index, edge_weight, use_teacher_forcing):
        ...
        x, idx, wgt = self.graph_conv(x, edge_index, edge_weight)
...
class OutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_split = torch.split(x, [24, 3, 9, 1, 7], dim=-1)  # 假设 node_num 可以划分为 5 块
        # 对每个块分别应用全连接层
        out_list = []
